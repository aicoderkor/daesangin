      ...
      {lodgingOpen && (
        <div className="modal on" onClick={(event) => { if (event.target === event.currentTarget) setLodgingOpen(false) }}>
          <div className="sheet lodging-sheet">
            ...
            <button type="button" className="btn" disabled={game.gold < 250 * game.facilities.quarters * game.facilities.quarters} onClick={() => { if (gameStore.upgradeFacility('quarters')) onToast?.('숙소 수용량이 증가했습니다.') }}>수용량 +1 · 250동</button>
          </div>
        </div>
      )}
    </>
